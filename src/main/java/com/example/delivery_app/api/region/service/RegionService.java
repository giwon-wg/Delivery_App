...
public class RegionService {
	...
	public RegionResponse getRegionData(double lat, double lng) {
		...
		if (region.startsWith("서울")) {
			...
			return new RegionResponse("부산광역시", "/logos/busan.png", "여봤나!", "돼지국밥, 밀면");
		} else if (region.startsWith("대구")) {
			return new RegionResponse("대구광역시", "/logos/daegu.png", "여기라꼬~", "막창, 납작만두");
			...
			return new RegionResponse("강원도", "/logos/gangwon.png", "여기더래요!", "감자옹심이, 막국수");
		} else if (region.contains("제주")) {
			return new RegionResponse("제주도", "/logos/jeju.png", "여긴게마씸~", "흑돼지, 고기국수");
		} else if (region.contains("경기도")) {
			return new RegionResponse("경기도", "/logos/gyeonggi.png", "여기쥬~", "소불고기 정식");
		} else if (region.contains("전북특별자치도"))
			return new RegionResponse("경기도", "/logos/jeolla.png", "여기쥬~", "소불고기 정식");

		return new RegionResponse(region, "/logos/default.png", "여기요!", "김치볶음밥");
	}
}